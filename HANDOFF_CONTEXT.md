...
- Company Rules has a `People Rules Health` panel next to `Google Status`. It checks `ASSIGNMENT RULES\seller_terms.csv` for missing/unreadable files, empty CSVs, invalid Seller Rate/Deduction values, overlapping Seller/Sheet Type value ranges, inactive Sheet Type companies, missing Sheet Type assignment companies, and shows parsed rates/deductions. Buttons refresh the status, open the terms folder, and copy diagnostic details.
